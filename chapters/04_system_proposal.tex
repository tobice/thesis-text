...
\section{Benefits of integration into LinkedPipes Visualization}

\textit{Composed views vs LinkedPipes approach. Perhaps part }

\textit{Using LinkedPipes Visualization: pros and cons, re-used features (we need to analyze RDF data, it is already done for us) , contribution (! explicitly name what is done better), implementation stuff, limiting to a single type of application (domain specific, not combining views)}
...
\textit{Perhaps last part: how we want to achieve all of this. Describe how we are going to extend LinkedPipes. Replace the UI, configurator vs application. Mock-ups But that might rather be part of the implementation part.}	
